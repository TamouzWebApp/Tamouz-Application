...
class LocalStorageService {
    ...
    async loadInitialEvents() {
        try {
            const eventsFilePath = window.getEventsFilePath ? window.getEventsFilePath() : 'JSON/events.json';
            ...
            if (response.ok) {
                ...
                window.dispatchEvent(new CustomEvent('localStorageeventsUpdated', {
                    detail: { events, source: 'json' }
                }));
                
                console.log(`✅ Loaded ${events.length} events successfully`);
            } else {
                throw new Error(`Failed to load ${eventsFilePath}: ${response.status}`);
            }
        } catch (error) {
            console.warn(`⚠️ Failed to load events file:`, error);
            ...
            // إرسال حدث تحديث البيانات
            window.dispatchEvent(new CustomEvent('localStorageeventsUpdated', {
                detail: { events: demoEvents, source: 'demo' }
            }));
...
>>>>>>> 412d59bb4c7d071bf3929587c37cfb7a636b9e1b
            ...
            console.warn('⚠️ Failed to load JSON/users.json:', error);
    ...
    saveEvents(events) {
        ...
        try {
            ...
            // إرسال حدث إضافي للتحديث في الوقت الفعلي
            window.dispatchEvent(new CustomEvent('dataManagereventsRealTimeUpdate', {
                detail: { events, source: 'localStorage' }
            }));
    ...
    getEvents() {
        try {
            ...
            if (stored) {
                ...
                // تحقق من أن البيانات هي مصفوفة مباشرة أو داخل كائن events
